...
public class send : MonoBehaviour
{
    ...
    public void sendFinalScor(bool winner)
    {
        if (winner) // if winner = true, local client wins
            ...
            message = "!scor@" + opponentIP; //we use host and remote client interchangably
        sendString(message, 1);
    } //only host
    public void sendRankJoin()
    {
        if (isHost == true)
            message = "!join#" + myIP;
        sendString(message, 1);
    ...
    void Start()
    {
        ...
        remoteEndPoint[1] = new IPEndPoint(IPAddress.Parse(serverIP), 5000); // for now 5000
